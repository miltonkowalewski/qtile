...
def w_layout():
    return widget.CurrentLayout(**CurrentLayoutStyle(qtile_palette).stylize())

def w_current_layout_icon(**config):
    return extra_widget.CurrentLayoutIcon(**config)
...
w_net_graph = widget.NetGraph(
    graph_color="70ff70",
    fill_color=qtile_palette.active,
    border_color=qtile_palette.active,
)
w_memory = widget.Memory(
    foreground=qtile_palette.active,
    mouse_callbacks={"Button1": lambda: qtile.cmd_spawn(terminal or "alacritty" + " -e htop")},
    fmt="Mem: {}",
    padding=5,
)
w_memory_graph = widget.MemoryGraph(
    graph_color="70ff70",
    fill_color=qtile_palette.active,
    border_color=qtile_palette.active,
)
w_cpu_graph = widget.CPUGraph(
    graph_color="70ff70",
    fill_color=qtile_palette.active,
    border_color=qtile_palette.active,
)
...
w_notify = widget.Notify(fontsize=11)
...
w_quick_exit = extra_widget.ScriptExit(foreground="70ff70")
